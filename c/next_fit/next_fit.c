...
mp_init(void *mem, size_t size, int flags)
{
	...
	if ( size < MP_SIZE + MP_MIN_BLOCK + NODE_SIZE )
		return NULL;
	...
	node->is_used = 0;
	node->next = OFF(end);
	mp->next_block = OFF(node);

	mp->nfree = mp->capacity = OFF(end) - mp->next_block;
	mp->nalloc = 0;
	return mp;
}
...
mp_alloc(mempool *mp, size_t size)
{
	...
	mp_node_t *start_node = PTR(mp->next_block);
	mp_node_t *node = start_node;
	int finish = 0;
	...
	for (;;) {
		...
		if ( ! next->is_used ) {
			...
			while ( ! next->is_used );
			node->next = OFF(next);
			if ( OFF(node) < mp->next_block && mp->next_block < node->next ) {
				mp->next_block = node->next;
				start_node = PTR(mp->next_block);
			}
		}
...
mp_realloc(mempool *mp, void *mem, size_t size)
{
	...
	if ( block_size <= capacity ) {
		...
		if ( next->is_used ) {
			...
			*new_node = *next;
			node->next = OFF(new_node);
			mp->nfree += cap;
			if ( OFF(next) == mp->next_block )
				mp->next_block = OFF(new_node);
		}
		return mem;
	}
	...
	if ( ! next->is_used ) {
		...
		if ( ! next->is_used ) {
			...
			if ( OFF(free_node) < mp->next_block && mp->next_block < free_node->next )
				mp->next_block = free_node->next;
		}
...
mp_check(mempool *mp)
{
	...
	assert(MP_SIZE <= mp->next_block && mp->next_block < end);
